---
title: '**openCR** 2.0 - open population capture--recapture'
...
```
```{r fig1plot, ref.label = "fig1", fig.width=8, fig.height=3.5, echo = FALSE} 
...
| `normal` | `move.a` | Bivariate normal kernel (Gaussian, BVN)|
| `exponential` | `move.a` | Bivariate negative exponential kernel (Laplace, BVE) |
| `t2D` | `move.a`, `move.b` | Bivariate $t$-distribution kernel (2Dt, BVT)|
| `annular` | `move.a` | non-zero only at centre and edge cells |
...
| annular | $p_0$ | | $p_0 \delta$, $r = 0$ | $(1-p_0) c$|---|
|         |       | | $\frac{1-p_0}{\pi (c_2^2 - c_1^2)}$, $c_1 \le r < c_2$ |||
|         |       | | 0 otherwise |||
...
```{r sparsekernel, fig.width=7, fig.height=4}
par(mar=c(3,1,3,5))
