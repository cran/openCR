---
title: '**openCR** 1.5 - open population capture--recapture'
...
Models of the form 'JSSA...CL' are variations on the Pradel--Link--Barker models. **openCR** 1.5.0 recognises labels of the form 'PLB...' as an alias for each of these models. Thus 'PLBf' is synonymous with 'JSSAfCL', and 'PLBsecrl' is synonymous with 'JSSAsecrlCL'.
...
| `uniform` | (none) | Uniform within arbitrary kernel radius |
...
| exponential | $\alpha_l$ | | $\frac{1}{2 \pi \alpha_l^2} \exp \left( \frac{-r}{\alpha_l} \right)$ | $2 \alpha_l$ | ** |
...
The edge method is chosen by setting the argument 'edgemethod' in `openCR.fit`; the options are 'truncate' (default in 1.5.0), 'wrap', and 'none'. Wrapping is fast, but it will cause an error if the mask is not rectangular. If a rectangular mask does not make sense (e.g., because the habitat is patchy) then you must use `edgemethod = 'truncate'` for unbiased estimates. 
...
The `grain size' (see [RcppParallel](https://rcppcore.github.io/RcppParallel/)) may be varied with details$grain, but it seems to have little effect. 
...
Efford, M. G. (2020). secr: Spatially explicit capture--recapture models. R package version
  4.3.2. https://CRAN.R-project.org/package=secr/
