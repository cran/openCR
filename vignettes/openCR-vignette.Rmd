---
title: '**openCR** 1.1 - open population capture--recapture'
...
* Use `par.openCR.fit` (ncores > 1) to fit several models at once. This can save a lot of time.
* Don't assume that ncores > 1 will speed up `openCR.fit` itself: it may run slower than with ncores = 1 because of overheads in cluster setup and communication.
...
MARK is faster than **openCR** 1.0 for nonspatial models because it uses multiple threads. Multithreaded C++ code may be used in future versions of**openCR**.
...
3. Overdispersion adjustment (chat, QAIC) or GOF tests
4. MCMC
...
Parts of **openCR** are coded in C++, via the R package **Rcpp**, whereas **secr** uses C. This leaves a future pathway for the use of multiple threads. **openCR** also duplicates some C++ functions in native R code, which is useful for debugging. Select the R version by setting `details = list(R = TRUE)` in `openCR.fit`. This currently works for most models except those with detector type 'multi'.
...
&= \sum_{b=1}^J \sum_{d=b}^J \left[ \beta_{b-1} \prod_{j=b}^{j=d-1} \phi_j (1-\phi_d)
\, a ^ {d-b-1} \prod_{j=b}^{j=d} \int_{\mathbb R^2} \mbox{Pr} (\omega_{i,j} | \vec x_b) \, d \vec x_b \right]. \nonumber
\end{align}
...
&= \sum_{b=1}^J \sum_{d=b}^J \left[ \beta_{b-1} \prod_{j=b}^{j=d-1} \phi_j (1-\phi_d)
\, a ^ {d-b-1} \prod_{j=b}^{j=d} \int_{\mathbb R ^2} \mbox{Pr} (\omega_{i,j} | \vec x_j) \,.\, f(\vec x_j |  \vec x_{j-1}, \omega_{i,j-1}) \; d \vec x_j \right].
\end{align}
...
|----------|----------------------------------------|
| Static | $a^{d-b+1} \int_{\mathbb{R}^2}  \prod _{j=b}^{j=d} p(\omega_{i,j}$\|$\vec x)\; d\vec x$ |
| Uncorrelated |  $a^{d-b+1}  \prod _{j=b}^{j=d} \int_A  p(\omega_{i,j}$\|$\vec x) \; d\vec x$ |
| Correlated | $a^{d-b+1} \prod_{j=b}^{j=d} \int_{\mathbb{R}^2} p(\omega_{i,j}$\|$\vec x_{i,j}) f(\vec x_{i,j}$\|$\vec x_{i,j-1}, \omega_{i,j-1}) \; d \vec x_{i,j}$ |
...
[secr-finitemixtures.pdf]: http://www.otago.ac.nz/density/pdfs/secr-datainput.pdf
