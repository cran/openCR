...
make.kernel <- function (movementmodel = c('normal','exponential','t2D','uniform'),
    ...
    kernelp <- fillkernelp (1, movemodel-2, kernel, usermodel, cellsize=1, moveargsi, moveargs, FALSE)[,1]
...
getpstring <- function (movemodel, move.a, move.b, sep = ' ') {
    npar <- switch(movemodel, static = 0, uncorrelated = 0, normal = 1, exponential = 1, t2D = 2, 0)
    pstring <- ''
...
plot.kernel <- function (x, contour = FALSE, levels = NULL, text = FALSE, ...) {
    ...
    movemodel <- attr(x, 'movementmodel')
    npar <- switch(movemodel, static = 0, uncorrelated = 0, normal = 1, exponential = 1, t2D = 2)
    pstring <- getpstring(movemodel, move.a, move.b)
    ...
    title <-  paste('kernel = ', movemodel, ' spacing =', spacing, ' kernelradius =', k2, ' ', pstring)
